...
void runLock()
{
  digitalWrite(ON_OFF_GPS_local, HIGH);
  digitalWrite(MOSFET_SIM_local, HIGH);
  melodyConnect();
}
void restartDevice()
{
...
String handleCommands(const String& command) {
    ...
    if (command == "AT") {
        response = "OK";    
        melodyOK();  // Supone una función que reproduce un tono de confirmación    
    }
