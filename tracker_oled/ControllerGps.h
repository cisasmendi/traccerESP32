    ...
    while (ss.available() > 0) {
      if (gps.encode(ss.read())) {  
        if (gps.location.isValid()) {
          enviarDatos(gps.location.lat(), gps.location.lng());
        }
      }
    }
...
void enviarDatos(double lat, double lon) {
  String url = "http://104.248.226.93:5055/?lat=" + String(lat, 6) + "&lon=" + String(lon, 6) + "&id=863576045252975";
...
// Fun1ción para enviar comandos AT y esperar la respuesta
